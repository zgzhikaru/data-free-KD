...
parser = argparse.ArgumentParser(description='dfnd knowledge distillation')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    args.logger = datafree.utils.logger.get_logger(log_name, output='checkpoints/datafree-dfnd/log-%s-%s-%s-%s%s.txt'%(args.dataset, args.unlabeled, args.teacher, args.student, args.log_tag))
    args.tb = SummaryWriter('tb_log/datafree-dfnd/tensorboard-%s-%s-%s-%s%s'%(args.dataset, args.unlabeled, args.teacher, args.student, args.log_tag))
    if args.rank<=0:
    ...
    # 噪音矩阵
    noise_adaptation = torch.nn.Parameter(torch.zeros(num_classes,num_classes-1))
    optim_noise = torch.optim.Adam([noise_adaptation], lr=0.001)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train( train_loader, val_loader, [student, teacher], criterion, [optim_s, optim_noise], sched_s, epoch, args, noise_adaptation)
        ...
        args.logger.info('[Eval] Epoch={current_epoch} Acc@1={acc1:.4f} Acc@5={acc5:.4f} Loss={loss:.4f} Lr={lr:.4f}'
                .format(current_epoch=args.current_epoch, acc1=acc1, acc5=acc5, loss=val_loss, lr=optim_s.param_groups[0]['lr']))
        ...
        _best_ckpt = 'checkpoints/datafree-dfnd/%s-%s-%s-%s.pth'%(args.dataset, args.unlabeled, args.teacher, args.student)
...
def noisy(noise_adaptation, n_classes):
    if n_classes == 10:
...
def train(train_loader, val_loader, model, criterion, optimizer, scheduler, epoch, args, noise_adaptation):
    global best_acc1
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        with args.autocast():
            with torch.no_grad():
                t_out = teacher(images).detach()
            pred = torch.argmax(t_out, 1)

            s_out = student(images)

            loss_s = datafree.criterions.kldiv(s_out, t_out, T=args.T) * 4 

            s_out = F.softmax(s_out, dim=1)
            output_s_adaptation = torch.matmul(s_out, noisy(noise_adaptation, t_out.shape[1]))
            loss_s += nll(torch.log(output_s_adaptation), pred)
        optim_s.zero_grad()
        ...
        if args.print_freq>0 and i % args.print_freq == 0:
            ...
            args.logger.info('[Train] Epoch={current_epoch} Iter={i}/{total_iters}, train_acc@1={train_acc1:.4f}, train_acc@5={train_acc5:.4f}, train_Loss={train_loss:.4f}, Lr={lr:.4f}'
              .format(current_epoch=args.current_epoch, i=i, total_iters=len(args.kd_steps), train_acc1=train_acc1, train_acc5=train_acc5, train_loss=train_loss, lr=optim_s.param_groups[0]['lr']))
            loss_metric.reset(), acc_metric.reset()
        sched_s.step()
    datafree.utils.save_image_batch( args.normalizer(images, True), 'checkpoints/datafree-dfnd/%s-%s-%s-%s-ood-data.png'%(args.dataset, args.unlabeled, args.teacher, args.student) )
